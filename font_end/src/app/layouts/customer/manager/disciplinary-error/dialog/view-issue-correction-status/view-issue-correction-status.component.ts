...
@Component({
  templateUrl: './view-issue-correction-status.component.html',
  styleUrls: ['./view-issue-correction-status.component.css']
})

export class ViewIssueCorrectionStatusComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ViewIssueCorrectionStatusComponent>,
  ...
  async ngOnInit() {
    ...
    this.list_status = rs_status.data;
    this.formDisciplinaryError.issue_correction_status.status = rs_status.data.filter(s => s.code == this.formDisciplinaryError.issue_correction_status.status.code)[0];
