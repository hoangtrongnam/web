...
import { AddDisciplinaryStatusComponent, ViewDisciplinaryStatusComponent, ViewNotedErrorHandlingComponent } from '..';
...
@Component({
  selector: 'app-browse-disciplinary-status-updates',
  templateUrl: './browse-disciplinary-status-updates.component.html',
  styleUrls: ['./browse-disciplinary-status-updates.component.css']
})
export class BrowseDisciplinaryStatusUpdatesComponent implements OnInit {
  ...
  async view(item) {
    const dialogRef = this.dialog.open(ViewDisciplinaryStatusComponent, {
      width: '1200px',
      data: item
    });
  ...
  async clickAllChecked() {
    if (this.allFlag == false) {
      this.dataTable.forEach(s => s.disciplinary_status.approved.flag = true);
    }
    else {
      this.dataTable.forEach(s => s.disciplinary_status.approved.flag = false);
    }
  }
  async onSubmitSearch() {
    this.formNotedErrorHandling.disciplinary_status = new DisciplinaryStatusModel();
    this.formNotedErrorHandling.disciplinary_status.status = new StatusModel();
    this.formNotedErrorHandling.disciplinary_status.status.code = "2"
    this.formNotedErrorHandling.disciplinary_status.approved = new StatusModel();
    this.formNotedErrorHandling.disciplinary_status.approved.code = "0"
  ...
  async onSave() {
    var rs_data = await this._disciplineErrorService.updateListDisciplinaryStatus(this.dataTable);
    if (rs_data.code == "200") {
